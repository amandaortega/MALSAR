...
%% Code starts here
function [W, funcVal] = Least_SRMTL3(X, Y, S, rho1, rho2, opts)
...
% private functions
    ...
    function [grad_W] = gradVal_eval(W, rho1)
        ...
        term_reg = [];
        for i=1:task_num,
            term_aux = W(:,i)-W*B(:,:,i)*v_ones_T;
            for j=1:task_num,
                if j ~= i,
                    term_aux = term_aux + B(i,i,j)*(-W(:,j)+W*B(:,:,j)*v_ones_T);
                end
            end
            term_reg = [term_reg term_aux];
        end
        %*******************************
        % Eu acho que � 2*grad_W logo depois do igual, mas ele implementou
        % como grad_W. Teste com os dois, come�ando com a minha proposta.
        %*******************************
        grad_W = 2 * grad_W + rho1 * 2 *  term_reg...
            + rho_L2 * 2 * W;
    end
    ...
    function [funcVal] = funVal_eval (W, rho1)
        ...
        if opts.pFlag
            for i = 1: task_num
                ...
                for j = 1:task_num,
                    if j ~= i,
                        v_aux = v_aux + S(i,j)*W(:,j);
                    end
                end
                funcReg = funcReg + 0.5 * norm (W(:, i) - v_aux)^2;
            end
        else
            for i = 1: task_num
                funcVal = funcVal + 0.5 * norm (Y{i} - X{i}' * W(:, i))^2;
